...
const forWardTopMessage = async (client: TelegramClient, messageList: Api.Message[], channelname = '', top: string) => {
  ...
  await client.sendMessage("me", { message: `[THM] ${(new Date).toLocaleString()} start forward top ${top} messages from Telegram-Hottest_Message...` });
  await client.forwardMessages('me', { messages: topMessages, fromPeer: channelname });
  await client.sendMessage("me", { message: `[THM] ${(new Date).toLocaleString()} end forward top ${top} messages from Telegram-Hottest_Message.` });
